...
def main():
    ...
    for slug, occs in sorted(by_slug.items()):
        ...
        # Cards clamp the blurb to three lines in CSS and the details dialog
        # shows the rest, so storing the full text costs nothing on screen.
        # The cap is only a guard against a runaway page body.
        if len(blurb) > 700:
